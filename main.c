...
int Init(char *s) {
  ...
  GRAPHE=ImageEnGraphe(image);//on initialise la conversion de l'image en graphe
...
void menuFunc(int item) {
  ...
  case 0:
    ...
    break;
  case 1:
    GrapheEnImage(GRAPHE,image);
    break;
  case 2:
    MC = ComposanteConnexe(GRAPHE); 
    MC = Suppr4V(GRAPHE,MC);
    //print_MC(GRAPHE);
    break;
  case 3:
...
int main(int argc, char **argv) {  
  ...
  glutAddMenuEntry("Afficher les borres", 1);
  glutAddMenuEntry("Composante Connexe", 2);
  glutAddMenuEntry("", 3);
  glutAddMenuEntry("Sauver", 4);
  glutAddMenuEntry("Informations", 5);
